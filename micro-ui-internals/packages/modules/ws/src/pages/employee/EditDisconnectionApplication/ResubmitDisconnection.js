...
const ResubmitDisconnection = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      if (isAppDetailsPage) window.location.assign(`${window.location.origin}${details?.url}`);
    }, 5000);
  ...
  const onSubmit = async (data) => {
    ...
    let reqDetails = convertAppData.serviceType == "WATER" ? { WaterConnection: convertAppData } : { SewerageConnection: convertAppData }
...
export default ResubmitDisconnection;
